...
class ClientOrdersMapController extends GetxController {
  ...
  CameraPosition initialPosition =
      const CameraPosition(target: LatLng(19.4040535, -102.0463693), zoom: 14);
  ...
  void listenPosition() {
    socket.on('position/${order.id}', (data) {
      addMarker(
          ...
          data['latitude'],
          data['longitude'],
          'Tu repartidor es: ${order.delivery?.name ?? 'Desconocido'} ${order.delivery?.lastname ?? 'Desconocido'}',
          '',
          deliveryMarker!);
    });
  }
  ...
  void listenToDelivered() {
    socket.on('delivered/${order.id}', (data) {
      Fluttertoast.showToast(
          msg: 'Orden entregada.', toastLength: Toast.LENGTH_LONG);
  ...
  void updateLocation() async {
    try {
      ...
      animateCameraPosition(
          order.latitude ?? 19.4040535, order.longitude ?? -102.0463693);
      addMarker(
          'delivery',
          order.latitude ?? 19.4040535,
          order.longitude ?? -102.0463693,
          'Tu repartidor',
          '',
          deliveryMarker!);
      ...
      LatLng from =
          LatLng(order.latitude ?? 19.4040535, order.longitude ?? -102.0463693);
